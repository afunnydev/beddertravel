...
import makeSelectBusinessAddPage from './selectors';
import reducer from './reducer';
import saga from './saga';
import BusinessAddPageHeader from './BusinessAddPageHeader';

import BusinessAddGeneralInformation from '../BusinessAddGeneralInformation';
import BusinessAddGeneralInformationMore from '../BusinessAddGeneralInformationMore';
import BusinessAddBedrooms from '../BusinessAddBedrooms';
import { withConnect as withReduxConnect } from '../BusinessAddPageRedux';
import { withConnect as withGeneralReduxConnect } from '../BusinessAddGeneralInformationRedux';
import { withConnect as withBedroomsReduxConnect } from '../BusinessAddBedroomsRedux';
import BusinessAddPageRedux from '../BusinessAddPageRedux';
...
export class BusinessAddPage extends React.Component {
  constructor(props) {
    ...
    this.vRefs = BedderValidator.makeRefs(BedderValidator.getBusinessAddPage());
    ...
    return (
      <div>
        ...
        <Grid container style={{ padding: '0px 15px' }}>
          <BusinessAddPageHeader />
          
          {this.props.submitting 
            ? <BedderLoadingIndicator full />
            : businessStatus >= 2
              ? <Grid container spacing={4}>
                <BusinessAddGeneralInformation
                  refresh={refresh}
                ...
                <BusinessAddGeneralInformationMore
                  refresh={refresh}
                  width={this.props.width}
                  isMobile={isMobile}
                  isXS={isXS}
...
BusinessAddPage.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

BusinessAddPage.defaultProps = {
  submitting: true,
};

const mapStateToProps = createStructuredSelector({
  businessaddpage: makeSelectBusinessAddPage(),
});
...
const withReducer = injectReducer({ key: 'businessAddPage', reducer });
const withSaga = injectSaga({ key: 'businessAddPage', saga });
